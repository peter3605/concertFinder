...
func GetArtistResolution(ctx context.Context, pool *pgxpool.Pool, spotifyArtistID string) (ArtistResolution, bool, error) {
	...
	err := pool.QueryRow(ctx, q, spotifyArtistID).Scan(&r.SpotifyArtistID, &r.TicketmasterAttractionID, &r.ResolvedAt)
	if err != nil {
...
func UpsertArtistResolution(ctx context.Context, pool *pgxpool.Pool, r ArtistResolution) error {
	const q = `
INSERT INTO artist_resolutions (spotify_artist_id, ticketmaster_attraction_id, resolved_at)
...
`
	_, err := pool.Exec(ctx, q, r.SpotifyArtistID, r.TicketmasterAttractionID)
	return err
}
